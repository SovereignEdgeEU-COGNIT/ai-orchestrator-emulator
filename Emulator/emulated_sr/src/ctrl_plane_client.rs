...
pub async fn register_host() -> Result<Config, Box<dyn std::error::Error>> {
    ...
    let my_local_ip = std::env::var("HOST_IP").unwrap();
    ...
    let host = Host{ip: my_local_ip.clone(), name: hostname, port: port};
    ...
    let _ = res.json::<u16>().await?;

    let ipv4 = my_local_ip.parse::<std::net::Ipv4Addr>().unwrap();
